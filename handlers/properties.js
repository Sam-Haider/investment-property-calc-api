...
exports.addProperty = async function (req, res) {
    try {
        ...
        const calculateMonthlyPayment = ({ loan, interest, term }) => {
            const monthlyInterest = (interest * .01) / 12
            const termInMonths = term * 12
            const r = 1 / (1 + monthlyInterest)
            const payment = Math.round(loan * ((1 - r) / (r - (Math.pow(r, termInMonths + 1)))))
            return payment
        }
        const mortgagePayment = calculateMonthlyPayment(mortgage)
        console.log('mortg', mortgagePayment)
        const cashFlow = (parseInt(req.body.rentalIncome) - mortgagePayment) - parseInt(req.body.expenses)
        const cocReturn = ((cashFlow * 12) / parseInt(req.body.downPayment)).toFixed(2) * 100
        console.log("cashflow", cashFlow, "coc return", cocReturn)
